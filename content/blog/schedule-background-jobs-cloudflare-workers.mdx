...
async function handleSignup(request: Request, env: Env) {
  ...
  const fliqResponse = await fetch("https://api.fliq.sh/jobs", {
    method: "POST",
    headers: {
...
```typescript
const response = await fetch("https://api.fliq.sh/jobs", {
  method: "POST",
  headers: {
    "Content-Type": "application/json",
    "Authorization": "Bearer " + env.FLIQ_API_TOKEN,
...
```bash
curl https://api.fliq.sh/jobs/job_abc123 \
