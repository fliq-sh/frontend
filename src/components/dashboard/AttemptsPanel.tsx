...
export function AttemptsPanel({ job }: { job: Job }) {
  ...
  return (
    <div className="flex flex-col gap-2 bg-white/[0.015] px-4 py-3 sm:px-5">
      {job.webhook_url && (
        <p className="text-xs text-white/40">
          <span className="font-medium uppercase tracking-wider text-white/30">Webhook</span>{" "}
          <span className="font-mono text-white/55">{job.webhook_url}</span>
        </p>
      )}
      ...
      {loading ? (
        ...
        <p className="py-1 text-xs text-white/35">No attempts yet — this job hasn&apos;t run.</p>
      ) : (
        <ul className="flex flex-col gap-1">
          {attempts.map((a) => {
            ...
            return (
              <li
                key={a.id}
                className="flex flex-wrap items-center gap-x-4 gap-y-1 rounded-md border border-white/5 bg-white/[0.02] px-3 py-2 text-xs"
              >
                <span className="font-mono text-white/50">#{a.attempt_num}</span>
                {a.status_code ? (
                  <StatusPill tone={tone} label={`HTTP ${a.status_code}`} />
                ) : (
                  <StatusPill tone="neutral" label="no response" />
                )}
                <span className="text-white/45">{formatDuration(a)}</span>
                <RelativeTime date={a.started_at} className="text-white/35" />
                {a.error && (
                ...
                <Link
                  ...
                  className="ml-auto inline-flex items-center gap-1 text-white/40 transition-colors hover:text-white"
