...
export function MethodChip({ method, className }: { method: string; className?: string }) {
  return (
    <span
      className={cn(
        "inline-flex items-center rounded border border-foreground/15 bg-foreground/5 px-1.5 py-0.5 text-[10px] font-mono font-semibold tracking-wide text-foreground/80",
        className,
      )}
    >
      {method.toUpperCase()}
